...
const Cart = () => {
  ...
  const renderCart = () => {
    return cartState.map((val, index) => (
      <div
        className='bg-white shadow-md h-44 my-5 rounded-lg flex overflow-hidden'
        key={index + 1}
      >
        <div className='w-11/12 p-4 phone:p-4 flex items-center'>
          <img
            ...
            alt={val.productName}
            className='h-full w-40 rounded-lg mr-8'
          />
          ...
          <div className='mr-8'>
            {val.qty >= val.stock ? (
              <button
                disabled
                className='bg-white h-10 w-10 cursor-not-allowed'
              >
                +
              </button>
            ) : (
              <button
                className='bg-white hover:bg-peach-dark h-10 w-10'
            ...
            <input
              type='text'
              className='h-10 w-14 focus:outline-none text-center'
              value={val.qty}
            />
            {val.qty <= 1 ? (
              <button
  ...
  return (
    <div>
      ...
      {cartState.length ? (
        <>
          <div className='px-20 phone:px-4 mb-28'>{renderCart()}</div>
          <CartFooter />
        </>
      ) : (
