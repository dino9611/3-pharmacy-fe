...
const UserPrescription = () => {
    ...
    const UploadHandler = async(e) => {
        if(e.target.files[0]){
            ...
            try {
              ...
              await axios.patch(`${API_URL}/custom/update/payment/${idPrescription}`, formData, config)
              let updateStatus = {id: idPrescription, nextStatus : "waitpaymentApproval"}
    ...
    const handleopenDialog = async (index) => {
        ...
        try {
            let results = await axios.get(`${API_URL}/custom/details/${dataCust[index]?.id}`)
                ...
                <DialogContent>
                    ...
                    <div className='flex justify-evenly'>
                        ...
                        <DialogContentText sx={{ml: 6 }} >
                            ...
                            <div className=' mt-12 text-yellow-500'>Please contact the admin if you see anything suspicious</div>
                        </DialogContentText>
                    </div>
                </DialogContent>
                <DialogActions>
                    ...
                    <Card sx={{ minWidth: 275, mb: 2.5 }}>
                        <CardContent>
                            <Typography variant="h5" component="div" sx={{mb:2}}>
                                {val.prescriptionName ? val.prescriptionName : "Data is still being processed"}
                            </Typography>
                            <Box sx={{ width: '100%' }}>
                                <Stepper 
                                // activeStep={dataStatus.findIndex(val => val.status === "delivered")} 
                                activeStep={val.status === "delivered" ? dataStatus.findIndex(x => x.status === val.status) + 1 : dataStatus.findIndex(x => x.status === val.status) } 
                                alternativeLabel
                                >
                        ...
                        <CardActions sx={{justifyContent: 'space-between'}}>
                            <Button onClick={() => handleopenDialog(index)} size="small">Details</Button>
                            {/* <Button onClick={() => dummyButt(index)} size="small">dummy</Button> */}
                            {val.status == 'waitingPayment' ? 
                                (<label htmlFor="contained-button-file">
                                    <Input accept="image/*" id="contained-button-file" multiple type="file" onChange={UploadHandler} />
                                    <Button startIcon={<FileUploadOutlinedIcon/>} size='small' onClick={() => uploadClick(index)} variant='outlined' component="span">
                                        Upload Payment
                                    </Button>
                                </label>) 
                                : 
