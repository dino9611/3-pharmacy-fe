...
const AdminPrescriptions = () => {
    ...
    const getCustom = async () => {
        try {
            let results = await axios.get(`${API_URL}/custom`, {params : {status}})
    ...
    const rejectHandler = (index) => {
        ...
        if (result.isConfirmed) {
            ...
            let updateStatus = {id : customData[index].id, nextStatus : statusReject}
            axios.patch(`${API_URL}/custom/nextStatus`, updateStatus)
            .then(() => {
                getCustom()
    ...
    const acceptHandler = async (index) => {
        try {
            ...
            let results = await axios.get(`${API_URL}/custom/medicine/${customData[index].id}`)
            results = await axios.get(`${API_URL}/custom/details/${customData[index]?.id}`)
    ...
    const commitHandler = async() => {
        try {
            let updateStatus = {id: customData[indexProduct].id, nextStatus : nextStatus}
    ...
    const dialogAccept = () => {
        ...
        return (
            <Dialog maxWidth={"md"} open={openAcceptDialog} onClose={closeAcceptDialog}>
                ...
                <DialogContent>
                    <div className='flex justify-evenly'>
                        ...
                        <DialogContentText sx={{ml: 6 }}>
                            ...
                            <FormControlLabel sx={{mt:10, color: '#66806A', fontSize: 7}} onChange={protectHandler} labelPlacement="end" control={<Checkbox />} 
                ...
                <DialogActions>
                    <Button onClick={closeAcceptDialog}  sx={{color: '#66806A'}}>Cancel</Button>
                    <Button sx={{color: '#66806A'}} disabled={commitProtect ? true : false } onClick={commitHandler} >Commit</Button>
                </DialogActions>
            </Dialog>
        )
    }
    ...
    const handleopenDialog = async (index) => {
        setindexProduct(index)
        try {
            let results = await axios.get(`${API_URL}/custom/details/${customData[index]?.id}`)
                ...
                <DialogContent>
                    ...
                    <div className='flex justify-evenly'>
                        ...
                        <DialogContentText sx={{ml: 6 }} >
                            {dataProduct?.prescriptionName ? 
                                    ...
                                    (  
                                        <>
                                            ...
                                            />
                                            <div className='text-red-700 mb-5'>Please fill the Prescription Name before continuing</div>
                                        </>
                                    )
                            }
                            <div className=' flex text-center mb-4'>
                            ...
                            <div className='text-right mt-12 text-yellow-500'>Please double check details product before you update status Data Product</div>
                        </DialogContentText>
                    </div>
                </DialogContent>
                <DialogActions>
                    ...
                    : 
                    (  <Button onClick={saveHandler} sx={{color: '#66806A'}} >Save</Button>)}
                </DialogActions>
            </Dialog>
        )
    }
            ...
            <TableBody sx={{backgroundColor: "#B4C6A6"}}>
                {customData.map((row, index) => (
                    ...
                    >
                        ...
                        <TableCell align="right">{row.prescriptionName ? row.prescriptionName : "No Name"}</TableCell>
                        <TableCell align="right">{row.paymentProof ? (
                            <Button onClick={() => imagePayment(index)}>Click Here</Button>) : "Belum Bayar"}
                        </TableCell>
                        <TableCell align="right">{dataStatus[dataStatus.findIndex(x => x.status === row.status)].display}</TableCell>
                        <TableCell align="right">
                            <Button onClick={() => handleopenCreate(index)} sx={{display : status == 'initial' ? 'block' : 'none' }} >Create Medicine</Button>
                        </TableCell>
                        <TableCell align="right">
                        ...
                        <TableCell align="right">
                            ...
                            : 
                            (<Button sx={{display : status == 'delivered' ? 'none' : 'block' }} onClick={() => acceptHandler(index)}>Accept</Button>)
                            }
    ...
    return (
        <div className="w-4/5 absolute right-0 p-2">
            {dialogUpdate()}
            {dialogAccept()}
            <CreatePrescription open={openCreate} handleClose={handleopenCreate} dataCustom={customData[indexProduct]} handleClosetest={handleCloseCreate} />
            <Box sx={{ width: '100%', bgcolor: 'background.paper' }}>
                <Tabs value={indexTab} onChange={handleChange} variant="fullWidth" scrollButtons="auto">
                ...
                <Table sx={{ minWidth: 650 }} size="medium" aria-label="a dense table">
                    <TableHead>
                        <TableRow sx={{backgroundColor: "#66806A"}}>
                            ...
                            <TableCell align="right" sx={{width: "15vw"}}>Prescription Name</TableCell>
                            <TableCell align="right" sx={{width: "15vw"}}>Payment Proof</TableCell>
                            <TableCell align="right" sx={{width: "15vw"}}>Status</TableCell>
                            <TableCell align="right" sx={{width: "15vw"}}></TableCell>
                            <TableCell align="right" sx={{width: "5vw"}}></TableCell>
                            <TableCell align="right" sx={{width: "5vw"}}></TableCell>
                            {rejected.includes(status) ? (<TableCell align="right" sx={{width: "5vw"}}></TableCell>) : (null)}
                        </TableRow>
                    </TableHead>
                    {renderBody()}
                </Table>
